...
//	delete pattern_;
   ...
   ~AbstractAdjacencyIterator()
   {
      if( helper_ != NULL ) delete helper_;
   }
   // ----------------------------------------------------------------------
   template<typename NodeType>
   shawn::EdgeModel::AbstractAdjacencyIterator<NodeType>&
   ...
   EdgeModel()
      : world_      ( NULL ),
        comm_model_ ( NULL )
   {
//	   			  	  std::cout << "EdgeModel" << std::endl << std::flush;

	   ccp[CD_IN] = new InCommPattern<const Node>(*this);
		ccp[CD_OUT] = new OutCommPattern<const Node>(*this);
		ccp[CD_BIDI] = new BiDiCommPattern<const Node>(*this);
		ccp[CD_ANY] = new AnyCommPattern<const Node>(*this);
		cp[CD_IN] = new InCommPattern<Node>(*this);
		cp[CD_OUT] = new OutCommPattern<Node>(*this);
		cp[CD_BIDI] = new BiDiCommPattern<Node>(*this);
		cp[CD_ANY] = new AnyCommPattern<Node>(*this);
/*	   for (int i =0; i<4; i++) {
		   std::cout << "EdgeModel: cp[" << i << "]=" << cp[i] << std::endl << std::flush; 
		   std::cout << "EdgeModel: ccp[" << i << "]=" << ccp[i] << std::endl << std::flush; 
	   }*/
