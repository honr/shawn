...
namespace shawn
{
   template<typename ccNodeType, typename ccWorldNodeIterator>
		shawn::LazyEdgeModel::LazyIteratorHelper<ccNodeType, ccWorldNodeIterator>::
			LazyIteratorHelper( const LazyEdgeModel& em,
					   CommunicationPattern<ccNodeType>& p,
                       ccNodeType& n,
                       const ccWorldNodeIterator& wit,
                       const ccWorldNodeIterator& wend )
      : AbstractIteratorHelper<ccNodeType>(p),
		edge_model_   ( em ),
        node_         ( n ),
        world_it_     ( wit ),
        world_end_it_ ( wend )
   ...
   init( void )
      throw()
   {
      while( (world_it_!=world_end_it_) && !(base_type::pattern_)(node_,*world_it_)) //!edge_model_.are_adjacent(node_,*world_it_) )
         ++world_it_;
   }
   // ----------------------------------------------------------------------
   template<typename NodeType,
   ...
   {
      return new
         LazyEdgeModel::LazyIteratorHelper
		 <NodeType,WorldNodeIterator>( edge_model_,
      ...
      const throw()
   {
      return new LazyIteratorHelper
		 ...
		 ( *this, *(get_communication_pattern(d)), v, world().begin_nodes(), world().end_nodes() );
   }
   // ----------------------------------------------------------------------
   EdgeModel::const_adjacency_iterator
   LazyEdgeModel::
      ...
      throw()
   {
      return new LazyIteratorHelper
         ...
         ( *this, *(get_communication_pattern_w(d)), v, world_w().begin_nodes_w(), world_w().end_nodes_w() );
   }
   // ----------------------------------------------------------------------
   EdgeModel::adjacency_iterator
   LazyEdgeModel::
