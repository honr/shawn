...
namespace shawn {
	...
	* These value are directly influenced by the parameter <DNF>doi</DFN>,
	...
	* This variable can be affected by the parameters <DFN>alpha</DFN> and 
	* <DFN>beta</DFN>. Furthermore, all values in the sequence are assured to be in the intervall from
	* <DFN>min_range</DFN> to <DFN>max_range</DFN>, comprehended as minimum / maximum value of percentage variation
	* in transmission range. If doi is set to zero, the RIM Model behaves like Disk Graph Model 
	* with transmission range <DFN>range</DFN>.<BR>
	* After generating the temporary KiSeq, <DFN>granularity</DFN> values are chosen and stored to the final <DFN>KiSeq_</DFN>.
	* The parameters <DNF>variance</DFN> and <DFN>mean</DFN> affect the behavior of a NormalRandomVariable. 
	* This Random Variable affects the maximum percentage variance of the signal sending power
	* for this model (Parameter <DFN>vsp</DFN>). The final vsp_ is set during construction
	* of the RIM Model.<BR>
   ...
   public:
	  ...
	  */
	   ...
	   //RimCommModel(double range,double min_range,double max_range,double doi,double granularity,double vsp);
      ~RimCommModel();
